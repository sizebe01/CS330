...
american_dict = set(line.strip() for line in open('/usr/share/dict/american-english', 'r'))
british_dict = set(line.strip() for line in open('/usr/share/dict/british-english', 'r'))
...
def getWords(perm, dictname):
    if dictname == "american-english":
        dictionary = american_dict
    else:
        dictionary = british_dict
    actualWords = []
    for item in perm:
...
def Posibilites(let1, let2, let3, let4, let5, let6, let7, exist = ""):
...
def scrabble():
    ...
    existingletters = existingletters.lower()

    wldcheck = [let1,let2,let3,let4,let5,let6,let7]
    if wldcheck.count('*') > 1:
        return render_template('results.html', error = 'Only one Wild Card is allowed!')


    check = request.form.get('attachExistingCheck')

    if check != None:
        pos = Posibilites(let1,let2,let3,let4,let5,let6,let6, exist = existingletters)
        ...
        pos = Posibilites(let1,let2,let3,let4,let5,let6,let6, exist = "")
        word_pos = getWords(pos,dictname)
        wordinfo = wordInformation(word_pos)
        wordsorted = sorted(wordinfo, key=operator.itemgetter(2), reverse=True)
